...
export default function GoogleSetupScreen() {
  const { verifyTwoFactorSetup } = useAuth();
  ...
  useEffect(() => {
    ...
    if (!flow || flow.type !== "setup") {
      setLocation("/login");
      return;
    }
    setQr(toQrImageSrc(flow.qr));
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setLoading(true);
    const result = await verifyTwoFactorSetup({ tempToken, code });
    setLoading(false);
      ...
      <Card className="w-full max-w-md bg-card/90 border-border/50 shadow-lg">
        ...
        <CardContent>
          <form className="space-y-5" onSubmit={handleSubmit}>
            <div className="flex justify-center rounded-xl border border-border/40 bg-secondary/20 p-4">
              <img src={qr} alt="Google Authenticator QR code" className="size-48 rounded-md" />
            </div>
